...
* моделируется двумерный конвективный перенос тепла, а также движение границы разделе фаз;
...
* Обозначения стандартные для задач теплофизики. Опорная температура: $T_0=\frac{T_{\max}+T_{\min}}{2}$.
...
* История содержит файлы с параметрами и методами управления запоминанием текущего состояния, включая сохранение и загрузку Истории на/с диск(а) в различных форматах (со сжатием данных).
...
* статус проекта: в активной разработке. Основная функциональность реализована, выполняются тестирование и подготовка к выпуску стабильной версии.
