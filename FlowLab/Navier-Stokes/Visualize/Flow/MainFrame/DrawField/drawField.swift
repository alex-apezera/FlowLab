...
extension Visualizator {
    ...
    func drawField(_ data: [[Double]], in context: inout GraphicsContext, scale: CGFloat, rx: [Double]) {
        ...
        // Вычисление лимитов для нормализации
        let (value_min, value_max) = fieldValueLimits(data)
        ...
        for j in 0..<ny-1 {
            let screenY_j = -CGFloat(solver.y[j] - halfLy) * scale
            let screenY_next = -CGFloat(solver.y[j+1] - halfLy) * scale
            
            for i in 0..<nx-1 {
                // Координаты 4-х углов ячейки с учетом РАЗНЫХ rx для разных j
                let p1 = CGPoint(x: CGFloat(solver.x[i] * rx[j] - halfLx) * scale,     y: screenY_j)
                let p2 = CGPoint(x: CGFloat(solver.x[i+1] * rx[j] - halfLx) * scale,   y: screenY_j)
                let p3 = CGPoint(x: CGFloat(solver.x[i+1] * rx[j+1] - halfLx) * scale, y: screenY_next)
                let p4 = CGPoint(x: CGFloat(solver.x[i] * rx[j+1] - halfLx) * scale,   y: screenY_next)
