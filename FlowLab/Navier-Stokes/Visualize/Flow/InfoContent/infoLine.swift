...
extension Visualizator {
    ...
    var infoLine: some View {
        HStack(spacing: 20) {
            Text("Процесс: \(isSolving ? "💡" : "🅿️")").bold()
            Text("Запуск: \(timerManager.formattedStartDate())")
            Text("Затрачено: \(timerManager.formattedElapsedTime())")
            Text("Память: \(SystemMonitor.getMemoryUsage, specifier: "%.f")Mb, CPU: \(SystemMonitor.getCPUUsage, specifier: "%.f")%")
           Spacer()
            Text("Кадров: \(history.frames.count)")
