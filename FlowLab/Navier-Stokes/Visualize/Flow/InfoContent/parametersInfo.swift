...
extension Visualizator {
    ...
    private var region: some View {
        HStack {
            ...
            Text("Размеры в [mm]: ").bold()
            Text("ширина: \(solver.params.Lx * 1000 , specifier: "%.0f"), высота: \(solver.params.Ly * 1000, specifier: "%.0f")")
            Text("Сетка: ").bold()
    ...
    private func displayParams(_ initTime: Double,_ fullTime: Double, _ dTime: Double, _ avg: Double, _ min: Double, _ max: Double, _ volumeChange: Double) -> some View {
        HStack {
            ...
            Text("Время: ").bold()
            Text("начальное (V₀=\(initVol, specifier: "%.2f")) \(formattedTime(initTime)),")
            Text("полное \(formattedTime(fullTime)),")
            Text("шаг \(formattedTime(dTime)).")
            Text("Расплав: ").bold()
            Text("объём V/V₀: \(avg, specifier: "%.2f"),")
            Text("(мин. \(min, specifier: "%.2f"),")
            Text("макс. \(max, specifier: "%.2f"));")
            Text("приращение объёма ")
