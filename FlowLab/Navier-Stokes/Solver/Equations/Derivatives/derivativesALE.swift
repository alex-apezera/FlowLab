...
//
//MARK: - Центрально-разностные производные второго порядка

extension NavierStokesSolver {
    
    // Производная по x
    @inline(__always)
    func derivativeX(_ array: [[Double]], _ j: Int, _ i: Int) -> Double {
        guard i > 0 && i < nx-1 else { return 0 }
        
        return (array[j][i+1] - array[j][i-1]) / ((x[i+1] - x[i-1]) * rx[j])
    }
    @inline(__always)
    func derivativePX(_ array: [Double], _ j: Int, _ i: Int) -> Double {
        guard i > 0 && i < nx-1 else { return 0 }
        
        return (array[idx(i+1,j)] - array[idx(i-1,j)]) / ((x[i+1] - x[i-1]) * rx[j])
    }

    // Производная по y
    @inline(__always)
    func derivativeY(_ array: [[Double]], _ j: Int, _ i: Int) -> Double {
        guard j > 0 && j < ny-1 else { return 0 }
        
        return (array[j+1][i] - array[j-1][i]) / (y[j+1] - y[j-1])
    }
    func derivativePY(_ array: [Double], _ j: Int, _ i: Int) -> Double {
        guard j > 0 && j < ny-1 else { return 0 }
        
        return (array[idx(i,j+1)] - array[idx(i,j-1)]) / (y[j+1] - y[j-1])
    }
