...
extension NavierStokesSolver {
    ...
    /// Схема Upwind 2-го порядка (LUD) - равномерная/неравномерная сеткка
    @inline(__always)
    func upwind2Ptr(phi: UnsafeBufferPointer<Double>, velocity: Double, _ j: Int, _ i: Int, _ idx: Int, axisX: Bool, _ nx: Int, _ ny: Int) -> Double {
        
        /// Множители 0.5 и 2 взаимо сокращены - см встроенные комменты
        if axisX {
            if stretch_x == 0.0 { /// равномерная сетка
            ...
            if stretch_y == 0.0 { /// равномерная сетка
