            ...
            DispatchQueue.concurrentPerform(iterations: workerCount) { wID in
                ...
                for j in startY..<endY {
                    ...
                    for i in 1..<nx-1 {
                        ...
                        /// (u·∇)T
                        let T_conv =
                        upwind2Enthalpy(phi: T_old, velocity: u[idx], j, i, idx, nx, nx_max, ny_max, inv2h, axisX: true) +
                        upwind2Enthalpy(phi: T_old, velocity: v[idx], j, i, idx, nx, nx_max, ny_max, inv2h, axisX: false)
                        
                        /// Запись в TNew через прямое обращение
                        buffer[idx] = T_old[idx] - dt * T_conv
                    }
                }
            }
        }}
        // Diffuse ∂T/∂t += α∇²T
        epmDiffusePar(quantity: &TNew) /// ⬅︎  α(T) учтено внутри
