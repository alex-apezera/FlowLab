...
extension NavierStokesSolver {
    ...
    func solveStep() async -> Bool {
        ...
        var attempts = 0
        let fatalError = "❌ Fatal error: Failed to stabilize the solution."
        ...
        if !isStepStable { await updateStatus(fatalError)
            return false // Прекращаем расчет
        }
        
        // Адаптация релаксации для давления
        if useAdaptiveRelax { adaptiveFactor(maxPressureResidual) }
        
        // Попытка ускорения при откатах ранее
        tryAcceleration()
        
        // Расчет объёма расплава,скорости плавления,тепловых потоков 
        useEnthalpyMethod ?
        ...
        // Баланс энергии
//        calculateEnergyBalance() /// запускается из MewScheme
    ...
    fileprivate func attemptRestoreState(_ attempts: inout Int) async {
        ...
        await updateStatus("⚠️ Rollback by \(bufferLimit) steps. Attempt \(attempts). dt = \(String(format: "%.2e",dt))")
    }
    
    ///  КОНТРОЛЬ: Проверка расходимости
